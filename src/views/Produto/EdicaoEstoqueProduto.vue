<template>
    <v-row>
        ...
        <v-col>
            ...
            <BottomNavigation :posicao="1" :produto="produto"></BottomNavigation>
    ...
    (async () => {
        try {
            const response = await axiosService.get(`/api/produto/${$route.params.id}`);
            Object.assign(produto.value, response.data.data);
            console.log("Produto do Back-End:", produto.value);
        } catch (error) {
    ...
    const submitProduto = async () => {
        ...
        try {
            const response = await axiosService.patch(`/api/produto/${$route.params.id}`, produto.value);
